...
//
// Rows absent from the file are reported as {id, name} but never deleted by this
// module. Deleting is the caller's decision and must be explicit, because removing a
// customer switches off check 2 for them entirely and turns their domain into an
// "unknown domain" warning in check 3. The workbook is reviewed in batches, so an
// absent row is often mid-review rather than gone.
...
function planImport(existing, incoming) {
  ...
  // Matched by domain, like everything else here: a renamed customer must not look
  // absent, or a rename would turn into a delete-and-recreate.
